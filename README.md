...
are required to run these scripts.  The generated files are in the
`test_files/data` directory, so the current test suite can be run
without having access to SAS or Stata.
